...
void start_server(SERVER_TYPE &server)
{
	...
	{
		// 从 istream 中获得字符串(*request.content)
		stringstream ss;
	...
	// 处理访问 /match/[字母+数字组成的字符串] 的 GET 请求，　例如执行 GET /match/abc123,　将返回 abc123
	...
	// 处理默认的 GET请求,如果没有其他匹配成功, 则这个匿名函数会被调用
	// 将应答 web/ 目录及其子目录中的文件
	// 默认文件 : index.html
	server.default_resource["^/?(.*)$"]["GET"] = [](ostream &response, Request & request)
	{
		...
		if(ifs)
		{
			...
			//　文件内容将拷贝到 response-stream 中，不应该用较大的文件
